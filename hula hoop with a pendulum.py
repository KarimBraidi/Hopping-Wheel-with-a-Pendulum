...
class Simulation:
    def __init__(
        ...
        #self.force = np.array([[0,-self.m*self.gr,0,-self.m_pendulum*self.gr*self.l_pendulum*np.cos(phi_init)]])
    ...
    def get_R(self,iter,X,prev_X,prev_AV,prev_q,prev_u,prev_gNdot,prev_gammaF,*index_sets):
        ...
        # With ndof=4, a, U, Q are already 4D, no need to extend
        a_full = a
        U_full = U
        Q_full = Q
        ...
        gN = np.array([q[1]-self.R])
        gNdot = np.array([u[1]])
        gNddot = np.array([a[1]])
        WN = np.array([[0,1,0,0]]).T
        gammaF = np.array([u[0]+(self.R)*u[2]])
        gammaFdot = np.array([a[0]+(self.R)*a[2]])
        WF = np.array([[1,0,self.R,0]]).T
...
if __name__ == "__main__":
    # Example: hoop with rim mass imbalance, started in contact with a prescribed spin.
     # Define prescribed pendulum motion
    # Example: oscillating pendulum at fixed frequency
    phi0 = 2*np.pi/3       # initial angle offset (radians) — pendulum oscillates about this angle
    amplitude = 7 * np.pi  # oscillation amplitude (radians)
    omega_p = 0.1 * np.pi  # oscillation frequency (rad/s)

    def phi_func(t):
        """Prescribed pendulum angle (in radians) as function of time."""
        return phi0 + amplitude * np.sin(omega_p * t)
    
    def phi_dot_func(t):
        """Time derivative of prescribed angle."""
        return amplitude * omega_p * np.cos(omega_p * t)
    
    def phi_ddot_func(t):
        """Second time derivative of prescribed angle."""
        return -amplitude * omega_p**2 * np.sin(omega_p * t)
    
    # Create and run simulation
    # To prescribe phi, pass callable phi_func / phi_dot_func / phi_ddot_func.
    # To let phi swing freely, pass phi_func=<initial angle> (scalar) and
    # phi_dot_func=<initial angular velocity> (scalar or None for 0).
    test = Simulation(
        ntime=1000,
        mu_s=10**9,
        mu_k=0.27,
        ...
        y0=0.18,
        x0=0.0,
        phi_func=phi_func,         # scalar => free pendulum starting at phi0
        phi_dot_func=phi_dot_func,     # None => released from rest
